    ...
    try {
      if (action === 'signup') {
        ...
        if (data?.user) {
          ...
          const { error: updateError } = await auth.updateUserProfile(data.user.id, {
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
         ...
         <CardFooter className="flex-col gap-4">
            <Button variant="outline" className="w-full" onClick={() => handleAuthAction('google')} disabled={isAuthLoading}>
